...
import { MdKeyboardArrowUp, MdKeyboardArrowDown, MdOutlinePreview, MdAnalytics, MdOutlineAnalytics } from "react-icons/md";
...
const FnStepReportBuilder = () => {
  ...
  const [getTransfomedColumnData, setTrasformedColumnData] = useState([]);
  ...
  const handleDbConnection = async (connectionParameter) => {
  ...
  const handleNext = async (rightItems, getselectedItems, getSelectedColumn, rows, getjoinrows, query, option) => {
    if (getselectedConnections['savedConnectionItems'] && getselectedConnections.query_name) {
      ...
      if (option == 3 || option == 9) {
        if (!getselectedConnections['savedConnectionItems'] && !getselectedConnections.query_name) {
          ...
          setError({ "savedConnectionItems": "Please choose from the Saved Connections" })
        }
      }
    }
            ...
            {stepCounter.map((stepItems, stepIndex) => {
              return (
                <div className={`${getstepCount >= stepItems.id ? "d-flex flex-column align-items-center" : "d-flex flex-column align-items-center nextstep"}`} key={stepIndex} >
                  <div className={`${stepItems.id == 7 ? `number_counter_container_last_child` : 'number_counter_container'} ${getstepCount > stepItems.id ? "completedStep" : ''}`}>
                  <p className="m-0">{getstepCount > stepItems.id ? <FaRegCircleCheck /> : stepItems.id}yy</p>
                  </div>
                  <div>
        ...
        {getstepCount == 2 &&
          <div className="d-flex justify-content-between" >
            ...
            {stepCounter.map((stepItems, stepIndex) => {
              return (
                <div className={`${getstepCount >= stepItems.id ? "d-flex flex-column align-items-center" : "d-flex flex-column align-items-center nextstep"}`} key={stepIndex} >
                  <div className={`${stepItems.id == 9 ? `number_counter_container_last_child` : 'number_counter_container'} ${getstepCount > stepItems.id ? "completedStep" : ''}`}>
                    <p className="m-0">{getstepCount > stepItems.id ? <FaRegCircleCheck /> : stepItems.id}</p>
                  </div>
                  <div>
                    <p >{stepItems.stepName}</p>
      ...
      <>
        {
          ...
          <FnTabPage
            ...
            user={user} gettoastMessage={gettoastMessage} setToastMessage={setToastMessage} />
        }
            ...
            {getstepCount == 8 ?
              ...
              <>
                <FnBtnComponent children={"Next"} onClick={() => handleNext(rightItems, getselectedItems, getSelectedColumn, rows, getjoinrows, query)}
                  classname={`${getstepCount == 1 || getstepCount == 2 || getstepCount == 9 ? 'visually-hidden' : 'sc_cl_outline_button d-block'}`} />

                <FnBtnComponent children={"Query Wizard"} onClick={() => handleNext(rightItems, getselectedItems, getSelectedColumn, rows, getjoinrows, query, 3)}
                ...
                <FnBtnComponent children={"Custom Query"} onClick={() => handleNext(rightItems, getselectedItems, getSelectedColumn, rows, getjoinrows, query, 9)}
                ...
                <FnBtnComponent children={"Back"} onClick={() => { handlePrev(getstepCount == 2 || getstepCount == 9 ? 1 : getstepCount); setEdit(false) }}
                  classname={`${getstepCount == 2 || getstepCount == 9 ? '' : 'visually-hidden'} sc_cl_outline_button`} />
...
const FnConnectionSelection = ({ authTokens, getSavedConnections, setSelectedConnections, getselectedConnections, handleDBConnection,
  ...
  , setActiveButtons, gettoastMessage, setToastMessage }) => {

  const [getToast, setToast] = useState([])

  const handleCheckedConnection = (savedConnectionItems, row) => {

    setSelectedConnections({
      ...getselectedConnections, savedConnectionItems
    })
  };
  ...
  return (
    <div>
        ...
        <FormGroup>
          <div className=" sc_cl_field_alignment">
            ...
            <Form.Control
              ...
              value={getselectedConnections?.query_name || ""}
              name={"query_name"}
              ...
              autoComplete="off"
            />
            <span className="sc_cl_span red">{geterror && geterror.query_name && geterror.query_name}</span>
          </div>
        </FormGroup>
      </Form>
      ...
      <div className="">
        <div className=" sc_cl_field_alignment">
          <label>Saved Connections <sup className="text-danger fs-6">*</sup></label>
          <span className="sc_cl_span red">{geterror && geterror.savedConnectionItems && geterror.savedConnectionItems}</span>
        </div>
        <div className="d-flex gap-2">
          {
            getSavedConnections.map((savedConnectionItems, tableIndex) => {
              return (
                <div className="col-2 p-3 bg-white shadow-sm rounded d-flex flex-column align-items-start gap-2" key={tableIndex}>
                  <div className="d-flex gap-2">
                    <input
                      type="checkbox"
                      onChange={(e) => handleCheckedConnection(savedConnectionItems)}
                      checked={getselectedConnections?.savedConnectionItems &&
                        getselectedConnections?.savedConnectionItems.id === savedConnectionItems.id
                        || false}
                    />
        ...
        <div>
          <button onClick={() => handleConnectionQueryPost()} className="mt-2 sc_cl_outline_button">Save & Continue</button>
        </div>
...
const FnTableSelection = ({ getselectedConnections, authTokens, leftItems, rightItems, setLeftItems,
  setRightItems, getselectedLeftItems, getselectedRightItems, setSelectedLeftItems, setSelectedRightItems, getsavedQueryDefinition,
  setSavedQueryDefinition, setPostedTables }) => {
  ...
  const moveSelectedToRight = () => {
    ...
    setLeftItems(leftItems.filter(item => !getselectedLeftItems.includes(item.id)));
    setSelectedLeftItems([]);

  };

  const moveSelectedToLeft = () => {
    const selectedItems = rightItems.filter(item => getselectedRightItems.includes(item.id));
    setLeftItems([...leftItems, ...selectedItems]);
    setRightItems(rightItems.filter(item => !getselectedRightItems.includes(item.id)));
  ...
  return (
    <div className="col-12">
      ...
      {/* intermediate Save */}
      <div>
        <button onClick={() => { postselectedTables() }} className="mt-2 sc_cl_outline_button">Save & Continue</button>
      </div>
    ...
    <div>
      ...
      <div className="mt-3">
        <Table>
          ...
          <tbody>
            {
              getcolumnalias.map((aliasItems, aliasIndex) => {
                return (
                  <tr key={aliasIndex}>
                    <td>
                      <select
                      ...
                      >
                        ...
                        {getSelectedColumn.map((tableNameItems, tableNametableIndex) => (
                          <option key={tableNametableIndex}>
...
const FnColumnAggreation = ({ getSelectedColumn, authTokens, getsavedQueryDefinition, rows, setRows }) => {
  ...
  const aggregateFunctions = [
    ...
    { id: 4, key: "Minimum", value: "Min" },
    { id: 5, key: "Maximum", value: "MAX" },
            ...
            <tr key={index}>
              <td>
                <select
                  ...
                  name="selectedTable"
                  value={row.selectedTable || row.table_name}
              ...
              <td>
                <select
                  value={row.selectedColumn || row.column_name}
                  name="selectedColumn"
                  className={`form-select form-select-sm`}
              ...
              <td>
                <select
                  ...
                  value={row.selectedAttribute || row.agg_fun_name}
...
const FnQueryExecutor = ({ authTokens, getSavedConnections, setSelectedConnections, getselectedConnections, handleDBConnection, getconnectiondbstr
  , getquerydata, setQueryData, leftItems, setLeftItems, userSettings, getedit, setEdit, getstepCount, setstepCount }) => {
  ...
  const handleGetQueryData = async () => {
    ...
    if (fetchTableRequest.status === 200) {
      setQueryResult(fetchTablenResults["response_data"]);
  ...
  const handleSaveQuery = async (status) => {
    ...
    if (status) {
      ...
      if (custom_query_Request.status === 201) {
        ...
        // window.alert("Not Saved Check Backend for Error Message!", custom_query_Results);
      }
    }
  }
    ...
    <div>
      ...
      <div>
        <div className="d-flex gap-5">
          <h5 className="w-75">Query Name : <span className="text-primary">{getselectedConnections.query_name}</span></h5>
          <h5 className="w-50">Available Tables</h5>
        </div>
        <Form>
          <Form.Group className={"d-flex gap-4"}>
            ...
            <Form.Control
              ...
              className={"w-75"} // Additional class for custom styling (optional)
        ...
        <div className="align-items-center d-flex justify-content-start mt-2 sc_cl_div">
          <FnBtnComponent children={"Validate"} onClick={() => handleGetQueryData()} classname={"sc_cl_close_button"} />
          {(getedit ? (
            <>
            ...
            <>
              <FnBtnComponent children={"Save as Draft"} onClick={() => handleSaveQuery(false)} classname={"sc_cl_submit_button ms-2"} />
...
const FnQueryResults = ({
  ...
  const fnGetQueryDefinitionDetails = async () => {
    const querydefinitionDataRequest = await fetch(
      `${process.env.REACT_APP_SERVER_URL}/api/${dynamic_get_api}/${startingIndex}/${endingIndex}/${user.username}/${search}`,
      {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
    ...
    // ! Revan Code
    const finalSQLQuerey = querydefinitionDataResult.data.map(queryItems =>
      fnSQLbuilder(queryItems, tableDataResult, columnDataResult, joinTableDataResult, aggregateTableDataResult))

    setQueryResult(finalSQLQuerey)
  ...
  const fnSQLbuilder = (queryItems, tableDataResult, columnDataResult, joinTableDataResult, aggregateTableDataResult) => {
    ...
    const selectedColumns = columnDataResult
      ...
      .map((column) => {
        const isColumnJoin = joinTableDataResult.some(
          (join) => column.column_name === join.join_column_name1 || column.column_name === join.join_column_name2
        );

        const tableName = tableDataResult.find((table) => table.id === column.table_column_table_id).table_name;
        const columnName = column.column_name;

        const colFunction = column.col_function && column.col_function === 'None' ? `${column.col_function}` : ``;

        const aliasName = column.alias_name && column.alias_name !== 'None' ? ` AS ${column.alias_name}` : '';

        const aggregateData = aggregateTableDataResult.find(
          (aggregate) => aggregate.table_aggregate_column_id === column.id
        );
        ...
        if (isColumnJoin) {
          ...
          if (aggregateData) {
            ...
            selectedColumn = `${aggregateData.agg_fun_name}(${tableName}.${columnName})${aliasName}`;

          } else if (colFunction) {
            if (colFunction && aliasName) {
              // Handle the case where col_function and  alias is present
              selectedColumn = `${tableName}.${columnName} ${aliasName}`;
            } else {
              selectedColumn = `${colFunction}(${tableName}.${columnName})${aliasName}`;
            }
          } else {
            selectedColumn = `${tableName}.${columnName}${aliasName}`;
          }
    ...
    const selectClause = `SELECT ${selectedColumns.length > 0 ? selectedColumns.join(', ') : "*"} FROM ${selectedTables[0]}`;
    ...
    const joinClauses = joinTableDataResult
      ...
      .map((join) => {
        ...
        const columnNameOne = columnDataResult.find((column) => column.column_name === join.join_column_name1); //.column_name;
        const columnNameTwo = columnDataResult.find((column) => column.column_name === join.join_column_name2); //.column_name;
  ...
  const handlePostExecutableQuery = async (executableQuery) => {

    let smpl = executableQuery

    smpl += " LIMIT 10"

    setShowModal(true)

    let queryRequest = await fetch(
      `${process.env.REACT_APP_SERVER_URL}/api/get_execute_query_data`,
      {
        method: "POST",
        body: JSON.stringify(smpl),
        headers: {
          "Content-Type": "application/json",
          Authorization: "Bearer " + String(authTokens.access),
        },
    ...
    let queryResults = await queryRequest.json()

    setQueryDataResults(queryResults)
  }


  const handlePostExecutableQueryFullData = async (executableQuery) => {

    let queryRequest = await fetch(
      `${process.env.REACT_APP_SERVER_URL}/api/get_execute_query_data`,
      {
        method: "POST",
        body: JSON.stringify(executableQuery),
        headers: {
          "Content-Type": "application/json",
          Authorization: "Bearer " + String(authTokens.access),
        },
    ...
    if (queryRequest.status === 200) {
      setstepCount(10)
      setquerydatafullresults(queryResults)
    }
  }
  ...
  const handleReverseQuery = async (reverseQuery) => {
    ...
    if (reverseQuery.query_id) {
      ...
      const queryDefRequest = await fetch(
        `${process.env.REACT_APP_SERVER_URL}/api/get_connection_data/${reverseQuery.query_id}/`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
      ...
      const tableDataRequest = await fetch(
        `${process.env.REACT_APP_SERVER_URL}/api/get_save_table_data/${reverseQuery.query_id}/`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
      ...
      const columnDataRequest = await fetch(
        `${process.env.REACT_APP_SERVER_URL}/api/get_save_column_data/${reverseQuery.query_id}/`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
      ...
      const aggregateTableDataRequest = await fetch(
        `${process.env.REACT_APP_SERVER_URL}/api/get_aggregate_table_data/${reverseQuery.query_id}/`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
      ...
      // Fect Join
      const joinTableDataRequest = await fetch(
        `${process.env.REACT_APP_SERVER_URL}/api/get_join_table_data/${reverseQuery.query_id}/`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
      ...
      if (matchedConnection) {
        const processedData = {
          ...
          queryName: queryDefResult[0].query_name,
      ...
      let transformedData = aggregateTableDataResult.map(aggItem => {
        ...
        return {
          id: aggItem.id,
          agg_fun_name: aggItem.agg_fun_name,
          table_name: tableItem ? tableItem.table_name : "",
          column_name: columnItem ? columnItem.column_name : "",
          table_aggragate_query_id: aggItem.table_aggragate_query_id,
          table_aggregate_table_id: aggItem.table_aggregate_table_id,
          table_aggregate_column_id: aggItem.table_aggregate_column_id
        };
      });
      ...
      // let joinTransformData = joinTableDataResult.map(joinItems => {
      ...
      //   return {
      //     id: joinItems.id,
      //     agg_fun_name: joinItems.agg_fun_name,
      //     table_id_one: tableItemNameone.table_name,
      //     table_id_two: tableItemNametwo.table_name,
      //     column_name_one: columnItemNameone.column_name,
      //     column_name_two: columnItemNametwo.column_name,
      //     table_aggragate_query_id: joinItems.tab_join_query_id,
      //     tab_join_table_id_one: joinItems.tab_join_table_id_one,
      //     tab_join_table_id_two: joinItems.tab_join_table_id_two,
      //     table_aggregate_table_id: joinItems.tab_join_table_id_one,
      //     table_aggregate_column_id: joinItems.table_aggregate_column_id
      //   }
      // })
  ...
  //   setSharePermissionType(e.value)
  //   setShareUser({ ...getshareuser, permissio_type: e.value })
  // }
  ...
  return (
    <div className="query_container">
      <Card className="overflow-hidden border-0">
        ...
        <div className="sc_cl_div d-flex mx-2">
          <FnTableComponent
            data={getquerydefinition}
            csv_export={csvdata}
          ...
          <div className="sc_cl_div overflow-auto col-2">
            <Table
...
const FnTabPage = ({
  ...
  getedit, setEdit,
  geteditquery, setEditQuery, setRows, setJoinRows, setActiveButtons,
  getcolumnalias, setColumnAlias, gettoastMessage, setToastMessage, setLeftItems, user
}) => {
